...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(self.s_levels[level]):
                ...
                for prenode in action_node.prenodes:
    ...
    def add_literal_level(self, level):
        ...
        # with sets, they may be "added" to the set without fear of duplication.
        # However, it is important to then correctly create and connect all of
        # the new S nodes as children of all the A nodes that could produce them,
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_1_add_effects = node_a1.action.effect_add
        node_1_rem_effects = node_a1.action.effect_rem
        node_2_add_effects = node_a2.action.effect_add
        node_2_rem_effects = node_a2.action.effect_rem

        for node in node_1_add_effects:
            if node in node_2_rem_effects:
                return True
        for node in node_2_add_effects:
            if node in node_1_rem_effects:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_1_add_effects = node_a1.action.effect_add
        node_1_rem_effects = node_a1.action.effect_rem
        node_1_pos_preconds = node_a1.action.precond_pos
        node_1_neg_preconds = node_a1.action.precond_neg

        node_2_add_effects = node_a2.action.effect_add
        node_2_rem_effects = node_a2.action.effect_rem
        node_2_pos_preconds = node_a2.action.precond_pos
        node_2_neg_preconds = node_a2.action.precond_neg

        for node in node_1_add_effects:
            if node in node_2_neg_preconds:
                return True
        for node in node_1_rem_effects:
            if node in node_2_pos_preconds:
                return True
        for node in node_2_add_effects:
            if node in node_1_neg_preconds:
                return True
        for node in node_2_rem_effects:
            if node in node_1_pos_preconds:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_1_preconds = node_a1.parents
        node_2_preconds = node_a2.parents

        for node1 in node_1_preconds:
            for node2 in node_2_preconds:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        node_1_parents = node_s1.parents
        node_2_parents = node_s2.parents

        for node1 in node_s1.parents:
            for node2 in node_s2.parents:
                if not node1.is_mutex(node2) and not node2.is_mutex(node1):
                    return False
